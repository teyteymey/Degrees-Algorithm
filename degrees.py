...
def shortest_path(source, target):
    ...
    """
    '''We want to find the shortest path from source to target, so we can use a breadth-first search (BFS) algorithm.
    BFS is ideal for this problem because it explores all neighbors 
    at the present depth prior to moving on to nodes at the next depth level.
    
    Cons: can be memory intensive and slower, as it stores all nodes at the current level before moving to the next level, thus exploring more nodes.
    ...
    while True:
        ...
        node_to_explore = frontier.remove()
